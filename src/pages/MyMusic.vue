...
export default {
  ...
  methods: {
    getMsg(id) {
      getUserOfId(id)
        .then((res) => {
          this.username = res.consumer.username;
          this.getuserSex(res.consumer.sex);
          this.birth = this.attachBirth(res.consumer.birth);
          this.introduction = res.consumer.introduction;
          this.location = res.consumer.location;
          this.avator = res.consumer.avator;
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    getCollectSongs(id) {
      getSongOfId(id)
        .then((res) => {
          this.collectList.push(res.song);
        })
        .catch((err) => {
          console.log(err);
        });
